...
def main():
    ...
    with io.open(prediction, 'w', encoding='utf8') as f:
        ...
        #########test##########
        test_dataset = data_preprocess('test')
        test_char, test_sentence = test_dataset.get_input()
        test_char = test_char.reshape((test_char.shape[0],-1))
        test_in = np.hstack((test_char, test_sentence))
        ...
        for i in range(n_test):
            ...
            f.write(test_dataset.IDs[i] + '\t' + test_dataset.sentences_str[i] + prediction + '\n') 

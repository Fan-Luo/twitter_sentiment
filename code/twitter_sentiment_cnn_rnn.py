...
def twitter_cnn_rnn(char_vocabulary_size: int, word_vocabulary_size: int, word_len: int, sentence_length: int, n_outputs: int) -> keras.Model:
    ...
    char_cnn1 = Convolution1D(n_filters, 3, activation='relu', padding='valid')(dropper1) 
    ...
    concat = merge([avgpool, dropper2], mode='concat')
    ...
    print(model.summary())
    plot_model(model, show_shapes = True, to_file='cnn_rnn53.png')

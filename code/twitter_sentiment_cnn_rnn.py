...
def twitter_cnn_rnn(char_vocabulary_size: int, word_vocabulary_size: int, character_length: int, sentence_length: int, tag_num: int, n_outputs: int) -> Tuple[keras.Model, Dict]:
    ...
    model_sentence = Sequential()
    model_sentence.add(Embedding(word_vocabulary_size, output_dim=256, input_length=sentence_length)) 
    model_sentence.add(Bidirectional(GRU(128, return_sequences=True,dropout=0.2, recurrent_dropout=0.2))) 
    model_sentence.add(Bidirectional(GRU(50, dropout=0.2, recurrent_dropout=0.2)))  
    ...
    print(model.summary())
    plot_model(model, show_shapes = True, to_file='cnn_rnn11.png')
